 ...
 *         /drone/object_detection/ellipse_det
 *      3. 【发布】检测结果的可视化图像话题
 *         /drone/camera/rgb/image_ellipse_det
***************************************************************************************************************************/
#include <math.h>
#include <string>
#include <vector>
...
{ 
    ...
    // 发布调试消息
    msg_node_name = "/drone/message/ellipse_det";
    ...
    if (nh.getParam("camera_topic", camera_topic)) {
        ...
        camera_topic = "/drone/camera/rgb/image_raw";
    }
    ...
    if (nh.getParam("train_imlist", train_imlist)) {
        ...
        if (message_print)
            pub_message(message_pub, offboard_pkg::Message::NORMAL, msg_node_name, "train_imlist is" + train_imlist);
    } else {
        if (local_print)
            ROS_WARN("didn't find parameter train_imlist");
        if (message_print)
            pub_message(message_pub, offboard_pkg::Message::WARN, msg_node_name, "didn't find parameter train_imlist");
    ...
    if (nh.getParam("train_imdir", train_imdir)) {
        ...
        if (message_print)
            pub_message(message_pub, offboard_pkg::Message::NORMAL, msg_node_name, "train_imdir is" + train_imdir);
    } else {
        if (local_print)
            ROS_WARN("didn't find parameter train_imdir");
        if (message_print)
            pub_message(message_pub, offboard_pkg::Message::WARN, msg_node_name, "didn't find parameter train_imdir");
    ...
    if (nh.getParam("saving_path", saving_path)) {
        ...
        if (message_print)
            pub_message(message_pub, offboard_pkg::Message::NORMAL, msg_node_name, "saving_path is" + saving_path);
    } else {
        if (local_print)
            ROS_WARN("didn't find parameter saving_path");
        if (message_print)
            pub_message(message_pub, offboard_pkg::Message::WARN, msg_node_name, "didn't find parameter saving_path");
    ...
    // 接收开关话题
    switch_subscriber = nh.subscribe("/drone/switch/ellipse_det", 10, switchCallback);
...
#ifdef ELLIPSE_PUB
    // 发布椭圆检测结果的话题
    ellipse_pub = it.advertise("/drone/camera/rgb/image_ellipse_det", 1);
#endif
    // 椭圆检测结果，xyz
    pose_pub = nh.advertise<offboard_pkg::DetectionInfo>("/drone/object_detection/ellipse_det", 1);
