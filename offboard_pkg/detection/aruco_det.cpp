 ...
 *         /drone/object_detection/aruco_det
 *      3. 【发布】检测结果的可视化图像话题
 *         /drone/camera/rgb/image_aruco_det
***************************************************************************************************************************/
#include <pthread.h>
#include <thread>
#include <chrono>
...
#include <offboard_pkg/MultiArucoInfo.h>
...
std::string switch_node_name = "/drone/object_detection/aruco_navigation_switch";
// 接收消息，允许暂停检测
...
std::string msg_node_name = "/drone/message/aruco_det";
...
void messageEcho(std::string msg, MSG_TYPE _msg_type)
{
    if (MSG_TYPE::INFO == _msg_type)
    {
        if (local_print)
            ROS_INFO(msg.c_str());
        if (message_print)
            pub_message(message_pub, offboard_pkg::Message::NORMAL, msg_node_name, msg);
    }
    else if (MSG_TYPE::WARN == _msg_type)
    {
        if (local_print)
            ROS_WARN(msg.c_str());
        if (message_print)
            pub_message(message_pub, offboard_pkg::Message::WARN, msg_node_name, msg);
    }
    else if (MSG_TYPE::ERROR == _msg_type)
    {
        if (local_print)
            ROS_ERROR(msg.c_str());
        if (message_print)
            pub_message(message_pub, offboard_pkg::Message::ERROR, msg_node_name, msg);
    }
}
...
int main(int argc, char **argv)
{
    ...
    std::string camera_topic = "/drone/camera/rgb/image_raw";
    std::string camera_params_yaml;
    std::string output_pose_topic = "/drone/object_detection/aruco_det";
    std::string output_multi_pose_topic = "/drone/object_detection/multi_aruco_det";
    std::string output_image_topic = "/drone/camera/rgb/image_aruco_det";
    int dictionary_id(2);
    ...
    //【发布】检测得到的位置与姿态信息
    pose_pub = nh.advertise<offboard_pkg::ArucoInfo>(output_pose_topic.c_str(), 1);
    multi_pose_pub = nh.advertise<offboard_pkg::MultiArucoInfo>(output_multi_pose_topic.c_str(), 1);
    //【发布】检测结果图像
    ...
    while (ros::ok())
	{
        ...
        if (!frame.empty())
        {
            ...
            if (markerids.size()>0)
            {
                ...
                offboard_pkg::MultiArucoInfo multi_pose_now;
                multi_pose_now.num_arucos = markerids.size();
                for(unsigned int i = 0; i < markerids.size(); i++)
                {
                    offboard_pkg::ArucoInfo pose_now;
