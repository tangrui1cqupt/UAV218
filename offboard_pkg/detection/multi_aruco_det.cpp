 ...
 * SXH update 20230403  replace landpad_det_len with max_aruco_len*1.5
 *
 * 说明: 降落目标识别程序，降落板的尺寸为60cmX60cm
 *      1. 【订阅】图像话题 (默认来自usb_cam)
 *         /usb_cam/image_raw
 *      2. 【发布】目标位置，发布话题见 offboard_pkg/msg/DetectionInfo.msg
 *         /drone/object_detection/multi_aruco_det
 *      3. 【发布】检测结果的可视化图像话题
 *         /drone/camera/rgb/image_multi_aruco_det
***************************************************************************************************************************/
...
//-------------VISION-----------
Mat img;
offboard_pkg::DetectionInfo pose_now;
...
void cameraCallback(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    try {
        ...
        if (message_print)
            pub_message(message_pub, offboard_pkg::Message::ERROR, msg_node_name, "cv_bridge exception");
        return;
    }
...
{
    ros::init(argc, argv, "multi_aruco_det");
    ros::NodeHandle nh("~");
    image_transport::ImageTransport it(nh);

    // 发布调试消息
    msg_node_name = "/drone/message/multi_aruco_det";
    message_pub = nh.advertise<offboard_pkg::Message>(msg_node_name, 10);
    ...
    if (nh.getParam("camera_topic", camera_topic)) {
        ...
        if (message_print)
            pub_message(message_pub, offboard_pkg::Message::NORMAL, msg_node_name, "camera_topic is" + camera_topic);
    } else {
        if (local_print)
            ROS_WARN("didn't find parameter camera_topic");
        if (message_print)
            pub_message(message_pub, offboard_pkg::Message::WARN, msg_node_name, "didn't find parameter camera_topic");
    ...
    if (nh.getParam("camera_info", camera_info)) {
        ...
        if (message_print)
            pub_message(message_pub, offboard_pkg::Message::NORMAL, msg_node_name, "camera_info is" + camera_info);
    } else {
        if (local_print)
            ROS_WARN("didn't find parameter camera_info");
        if (message_print)
            pub_message(message_pub, offboard_pkg::Message::WARN, msg_node_name, "didn't find parameter camera_info");
        camera_info = "camera_param.yaml";
    }

    position_pub = nh.advertise<offboard_pkg::DetectionInfo>("/drone/object_detection/multi_aruco_det", 10);


    // 接收开关话题
    switch_subscriber = nh.subscribe("/drone/switch/landpad_det", 10, switchCallback);

    // 接收图像的话题
    image_subscriber = it.subscribe(camera_topic.c_str(), 1, cameraCallback);
    // 发布ArUco检测结果的话题
    landpad_pub = it.advertise("/drone/camera/rgb/image_multi_aruco_det", 1);
    

    sensor_msgs::ImagePtr msg_ellipse;

    //std::string ros_path = ros::package::getPath("prometheus_detection");
    std::string ros_path = ros::package::getPath("offboard_pkg");
    
    if (local_print)
        cout << "DETECTION_PATH: " << ros_path << endl;
    if (message_print)
        pub_message(message_pub, offboard_pkg::Message::NORMAL, msg_node_name, "DETECTION_PATH: " + ros_path);
    ...
    double max_aruco_len = camera_config["multi_aruco_max_aruco_len"].as<double>(); //这里用max_aruco_len替换landpad_det_len，他们对应关系是max_aruco_len*1.5=landpad_det_len，这样只用量出中心最大二维码的边长就可以了，而不用量实际打印的纸张的宽度。而且现在只用中心两个叠加二维码，不用边上四个二维码，图案有变动，这个landpad_det_len更不好确定了，所以这里用max_aruco_len，这样只用改yaml文件里面multi_aruco_max_aruco_len这个参数就可以了，不用改也不使用landpad_det_len参数了。
    // DEBUG
    ...
    while (ros::ok())
    {
        while (!getImageStatus() && ros::ok()) 
        {
            ...
            if (message_print)
                pub_message(message_pub, offboard_pkg::Message::NORMAL, msg_node_name, "Waiting for image.");
        ...
        if (switch_state != is_suspanded)
        {
            ...
            if (!is_suspanded)
            {
                ...
                if (message_print)
                    pub_message(message_pub, offboard_pkg::Message::NORMAL, msg_node_name, "Start Detection.");
            }
            else
            {
                if (local_print)
                ...
                if (message_print)
                    pub_message(message_pub, offboard_pkg::Message::NORMAL, msg_node_name, "Stop Detection.");
            }
        }
        ...
        if (markerids.size() > 0)
        {
            ...
            if (19 == markerids[tt])
            {
                ...
                aruco::estimatePoseSingleMarkers(markerCornersONE, max_aruco_len, camera_matrix, distortion_coefficients, rvecs, tvecs);
                aruco::drawAxis(img, camera_matrix, distortion_coefficients, rvecs[0], tvecs[0],max_aruco_len*0.5f);
            }
            else if (43 == markerids[tt])
            {
                vector<vector<Point2f> > markerCornersONE;
                markerCornersONE.push_back(markerCorners[tt]);
                aruco::estimatePoseSingleMarkers(markerCornersONE, max_aruco_len*0.1, camera_matrix, distortion_coefficients, rvecs, tvecs);
                aruco::drawAxis(img, camera_matrix, distortion_coefficients, rvecs[0], tvecs[0],max_aruco_len*0.1*0.5f);
            }
            else if (1 == markerids[tt] || 2 == markerids[tt] || 3 == markerids[tt] || 4 == markerids[tt])
            {
                vector<vector<Point2f> > markerCornersONE;
                markerCornersONE.push_back(markerCorners[tt]);
                aruco::estimatePoseSingleMarkers(markerCornersONE, max_aruco_len*0.2, camera_matrix, distortion_coefficients, rvecs, tvecs);
                aruco::drawAxis(img, camera_matrix, distortion_coefficients, rvecs[0], tvecs[0],max_aruco_len*0.2*0.5f);
            }
            ...
            {
                id_to8_t[0] = -(max_aruco_len + max_aruco_len*0.2) / 2.;
                id_to8_t[1] = (max_aruco_len + max_aruco_len*0.2) / 2.;
                id_to8_t[2] = 0.;
            }
