...
{
    ros::init(argc, argv, "t265_circle_cross_astar");
    ros::NodeHandle nh;
    ros::NodeHandle nh1("~");
...
//真机上应该不用再先发点了，因为手动切offboard嘛，等程序先运行起来已经发点了，再手动切offboard就够了。
  ...
  //  }
    ...
    if((abs(cur_position[0]) < 0.1)&&(abs(cur_position[1]) < 0.1)&&((abs(cur_position[2] - 1.0)) < 0.1)&&(ros::Time::now() - time_snap1 > ros::Duration(10.0)))
      {
       state_flag = 1;
      }
    }
    ...
    if((abs(cur_position[0]-(circle_center_pos_enu_store_avr[0] + 1)) < 0.1)&&(abs(cur_position[1]-circle_center_pos_enu_store_avr[1]) < 0.1)&&((abs(cur_position[2]-circle_center_pos_enu_store_avr[2])-1.0) < 0.1)&&(ros::Time::now() - time_snap2 > ros::Duration(6.0)))
     {
       state_flag = 3;
    ...
    //定点悬停多少秒之后再进入下一个状态  10秒
    if((abs(cur_position[0] - 3) < 0.1)&&(abs(cur_position[1]) < 0.1)&&((abs(cur_position[2] - 0.5)) < 0.1)&&(ros::Time::now() - time_snap4 > ros::Duration(7.0)))
      {      
        //发布astar目标点话题。
