...
//真机上应该不用再先发点了，因为手动切offboard嘛，等程序先运行起来已经发点了，再手动切offboard就够了。
  ...
  //  }
    ...
    if((abs(cur_position[0]) < 0.1)&&(abs(cur_position[1]) < 0.1)&&((abs(cur_position[2] - 0.7)) < 0.1)&&(ros::Time::now() - last_time > ros::Duration(8.0)))
      {
       state_flag = 1;
    ...
    if((abs(cur_position[0] - 1) < 0.1)&&(abs(cur_position[1]) < 0.1)&&((abs(cur_position[2] - 0.7)) < 0.1)&&(ros::Time::now() - time_snap1 > ros::Duration(8.0)))
      {
       state_flag = 2;
      }
   ...
   if(state_flag == 5)
   {
    ...
    //没有检测到二维码时保持悬停,重新检测到返回35厘米高的指点,目前暂时没用
    if(state_flag == 6)
    {
    pos_setpoint.type_mask = 0b100111111000;  // 100 111 111 000  xyz + yaw
    pos_setpoint.coordinate_frame = 1;
