...
//真机上应该不用再先发点了，因为手动切offboard嘛，等程序先运行起来已经发点了，再手动切offboard就够了。
  ...
  //  }
    ...
    arm_cmd.request.value = false; //这样是否就是上锁
    ...
    **************************************************************************************************/
    ...
    if((ros::Time::now() - time_state1 > ros::Duration(1.0))&&(first_time_flag == 1))
    { 

       x_data.push_back(aruco_pos_enu[0]);
       y_data.push_back(aruco_pos_enu[1]);
       first_time_flag = 2;
    }

    if((ros::Time::now() - time_state1 > ros::Duration(2.0))&&(first_time_flag == 2))
    { 
    ...
    if((ros::Time::now() - time_state1 > ros::Duration(3.0))&&(first_time_flag == 3))
    { 
    ...
    if((ros::Time::now() - time_state1 > ros::Duration(4.0))&&(first_time_flag == 4))
    { 
    ...
    if((ros::Time::now() - time_state1 > ros::Duration(5.0))&&(first_time_flag == 5))
    { 
    ...
    if((ros::Time::now() - time_state1 > ros::Duration(6.0))&&(first_time_flag == 6))
    { 
    ...
    if((ros::Time::now() - time_state1 > ros::Duration(7.0))&&(first_time_flag == 7))
    { 
    ...
    if((ros::Time::now() - time_state1 > ros::Duration(8.0))&&(first_time_flag == 8))
    { 
    ...
    if((ros::Time::now() - time_state1 > ros::Duration(9.0))&&(first_time_flag == 9))
    { 
    ...
    if((ros::Time::now() - time_state1 > ros::Duration(10.0))&&(first_time_flag == 10))
    { 
    ...
    if((x_data.size() == 10)&&(y_data.size() == 10))
    {
    //***************************ceres圆拟合*************************************************
   ...
   if(state_flag == 4)
   {
    ...
    pos_setpoint.coordinate_frame = 1;
    float A = 0.1;
    float w = 0.3; //w应该和小车上的一样吧，转一圈的时间是2π除以w
    pos_setpoint.position.x = x0y0r[0] + A*sin(w*(ros::Time::now().toSec() - last_time.toSec() )+1.5708);
    pos_setpoint.position.y = x0y0r[1] + A*cos(w*(ros::Time::now().toSec() - last_time.toSec() )+1.5708);
