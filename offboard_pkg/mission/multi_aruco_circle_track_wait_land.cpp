...
//真机上应该不用再先发点了，因为手动切offboard嘛，等程序先运行起来已经发点了，再手动切offboard就够了。
  ...
  //  }
    ...
    //定点悬停多少秒之后再进入下一个状态进行圆跟踪  10秒
    if((abs(cur_position[0] - 1) < 0.1)&&(abs(cur_position[1]) < 0.1)&&((abs(cur_position[2])-0.7) < 0.1)&&(ros::Time::now() - time_snap1 > ros::Duration(10.0)))
      {
       state_flag = 1;
   ...
   if(state_flag == 4)
   {
    ...
    if((ros::Time::now() - aruco_preland_enupos_record_time > ros::Duration(TRAS - 1))&&(abs(position_det[0]) < 0.05)&&(abs(position_det[1]) < 0.05)&&(num_regain > 2)&&(rc_thrust < 1250))
    {
       state_flag = 7;
    }
