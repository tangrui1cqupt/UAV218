...
{
    ros::init(argc, argv, "circle_cross");
    ros::NodeHandle nh;
    ros::NodeHandle nh1("~");
...
//真机上应该不用再先发点了，因为手动切offboard嘛，等程序先运行起来已经发点了，再手动切offboard就够了。
  ...
  //  }
    ...
    if((abs(cur_position[0]) < 0.1)&&(abs(cur_position[1]) < 0.1)&&((abs(cur_position[2] - 1.1)) < 0.1)&&(ros::Time::now() - last_time > ros::Duration(10.0)))
      {
       state_flag = 1;
      }
    }
    ...
    pos_setpoint.position.x = circle_center_pos_enu[0] - 2.2;
