...
int Astar::search(Eigen::Vector3d start_pt, Eigen::Vector3d end_pt)
{
  ...
  {
    pub_message(message_pub, offboard_pkg::Message::WARN, NODE_NAME, "Astar can't find path: goal point is occupied.");
    return NO_PATH;
  }
      ...
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
      {
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_)
        {
          ...
          if (expand_node == NULL)
          {
            ...
            {
                pub_message(message_pub, offboard_pkg::Message::WARN, NODE_NAME, "Astar can't find path: reach the max_search_num.\n");
                return NO_PATH;
            }
          }
          // 如果当前节点已被扩展过，则更新其状态
  ...
  pub_message(message_pub, offboard_pkg::Message::WARN, NODE_NAME, "max_search_num: open set empty.");
  return NO_PATH;
}
