...
Eigen::Vector3d pos_drone_cartographer;
Eigen::Quaterniond q_cartographer;
...
void cartographer_cb(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    //if (msg->header.frame_id == "t265_odom_frame")
    {
        pos_drone_cartographer = Eigen::Vector3d(msg->pose.position.x, msg->pose.position.y, msg->pose.position.z);
        // pos_drone_t265[0] = msg->pose.pose.position.x + pos_offset[0];
        // pos_drone_t265[1] = msg->pose.pose.position.y + pos_offset[1];
        // pos_drone_t265[2] = msg->pose.pose.position.z + pos_offset[2];

        q_cartographer = Eigen::Quaterniond(msg->pose.orientation.w, msg->pose.orientation.x, msg->pose.orientation.y, msg->pose.orientation.z);
        //Euler_t265 = quaternion_to_euler(q_gazebo);
        // Euler_t265[2] = Euler_t265[2] + yaw_offset;
        // q_t265 = quaternion_from_rpy(Euler_t265);
    }
...
{
    ros::init(argc, argv, "cartographer_to_mavros");
    ros::NodeHandle nh;
    ...
    ros::Subscriber cartographer_sub = nh.subscribe<geometry_msgs::PoseStamped>("/tracked_pose", 100, cartographer_cb);
    ...
    while(ros::ok()){
        ...
        ***************************************/
        ...
        vision.pose.position.x = pos_drone_cartographer[0];
        vision.pose.position.y = pos_drone_cartographer[1];
        vision.pose.position.z = pos_drone_cartographer[2];

        vision.pose.orientation.x = q_cartographer.x();
        vision.pose.orientation.y = q_cartographer.y();
        vision.pose.orientation.z = q_cartographer.z();
        vision.pose.orientation.w = q_cartographer.w();
